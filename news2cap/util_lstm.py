...
class entangledLSTMCell(nn.Module):
    def __init__(self,  input_size, factor_size, entangled_size, hidden_size,use_bias=True):

        super(entangledLSTMCell, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.use_bias = use_bias
    ...
    def reset_parameters(self):
        ...
        if self.use_bias:
            ...
            init.constant(self.bias_f.data, val=0)
            init.constant(self.bias_o.data, val=0)
            init.constant(self.bias_c.data, val=0)

    def forward(self, input_, hx, entangler):
        """
        Args:
            input_: A (batch, input_size) tensor containing input
                features.
        ...
        f = torch.mm(torch.mm(entangler,self.weight_b_ih_f)*torch.mm(input_,self.weight_c_ih_f),self.weight_a_ih_f)\
            +torch.mm(torch.mm(entangler,self.weight_b_hh_f)*torch.mm(h_0,self.weight_c_hh_f),self.weight_a_hh_f)+self.bias_f
        o = torch.mm(torch.mm(entangler,self.weight_b_ih_o)*torch.mm(input_,self.weight_c_ih_o),self.weight_a_ih_o)\
            +torch.mm(torch.mm(entangler,self.weight_b_hh_o)*torch.mm(h_0,self.weight_c_hh_o),self.weight_a_hh_o)+self.bias_o
        c_tilde = torch.mm(torch.mm(entangler,self.weight_b_ih_c)*torch.mm(input_,self.weight_c_ih_c),self.weight_a_ih_c)\
...
class LSTM(nn.Module):
    ...
    def __init__(self, cell_class, input_size, hidden_size, factor_size=0,entangled_size=0, num_layers=1,
    ...
    @staticmethod
    def _forward_rnn(cell, input_, length, hx, entangler=None):
        max_time = input_.size(0)
        output = []
        for time in range(max_time):
            if isinstance(cell, BNLSTMCell):
                h_next, c_next = cell(input_=input_[time], hx=hx, time=time)
            elif isinstance(cell, entangledLSTMCell):
                h_next, c_next = cell(input_=input_[time], hx=hx, entangler=entangler)
            else:
    ...
    def forward(self, input_, length=None, hx=None, entangler=None):
        if self.batch_first:
        ...
        for layer in range(self.num_layers):
            ...
            layer_output, (layer_h_n, layer_c_n) = LSTM._forward_rnn(
                cell=cell, input_=input_, length=length, hx=hx, entangler=entangler)
